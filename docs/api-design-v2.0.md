...
> 
> **v2.0 变更说明**：所有接口统一使用 POST 方法，接口功能通过 URL 路径（接口名称）区分
...
### POST /api/v2/user/password/update
...
### POST /api/v2/users/status/update
...
### POST /api/v2/modules/detail/get
...
### POST /api/v2/modules/detail/update
...
|--------|------|------|------|
| moduleId | string | 是 | 模组ID |
| title | string | 否 | 模组标题（可选，以下同） |
| subtitle | string | 否 | 副标题 |
| coverImage | string | 否 | 封面图URL |
| images | string[] | 否 | 详情图片列表 |
| categoryId | int | 否 | 分类ID |
| difficulty | int | 否 | 难度 |
| minPlayers | int | 否 | 最少推荐人数 |
| maxPlayers | int | 否 | 最多推荐人数 |
| durationHours | decimal | 否 | 预计时长 |
| description | string | 否 | 详细描述 |
| rulesSummary | string | 否 | 规则概述 |
| tags | int[] | 否 | 标签ID列表 |
...
### POST /api/v2/modules/submit-review
...
### POST /api/v2/modules/categories/list
...
### POST /api/v2/admin/modules/pending-review/list
...
### POST /api/v2/admin/modules/review
...
### POST /api/v2/admin/modules/list
...
**返回值**: 同POST /api/v2/modules/mine/list结构，增加 `authorName`, `reviewerName`, `reviewedAt` 等管理字段。
...
### POST /api/v2/gm/certification/apply
...
### POST /api/v2/admin/gms/certification/review
...
### POST /api/v2/admin/gms/internal-flag/update
...
### POST /api/v2/sessions/detail/update
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| sessionId | string | 是 | 团ID |
| moduleId | string | 否 | 模组ID |
| title | string | 否 | 团标题 |
| description | string | 否 | 补充说明 |
| scheduledTime | datetime | 否 | 计划开始时间 |
| estimatedEndtime | datetime | 否 | 预计结束时间 |
| minPlayers | int | 否 | 开团最少人数 |
| maxPlayers | int | 否 | 开团最多人数 |
| venueRoomId | int | 否 | 包间ID |
| pricePerPlayer | decimal | 否 | 人均费用 |
...
### POST /api/v2/sessions/confirm-start
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| sessionId | string | 是 | 团ID（状态须为full） |
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| sessionId | string | 是 | 团ID（状态须为confirmed） |
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| sessionId | string | 是 | 团ID（状态须为in_progress） |
...
### POST /api/v2/sessions/reschedule
...
### POST /api/v2/sessions/detail/get
...
### POST /api/v2/sessions/mine/gm/list
...
### POST /api/v2/sessions/mine/player/list
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| sessionId | string | 是 | 团ID（状态须为recruiting） |
...
### POST /api/v2/sessions/register/proxy
...
### POST /api/v2/sessions/registrations/confirm
...
### POST /api/v2/sessions/registrations/cancel
...
### POST /api/v2/sessions/registrations/list
...
### POST /api/v2/admin/sessions/list
...
**返回值**: 同POST /api/v2/sessions/list结构，增加管理字段 `paymentAmount`, `refundStatus`, `adminNotes`。

---

### POST /api/v2/admin/sessions/force-status
...
### POST /api/v2/ratings/module/submit
...
### POST /api/v2/ratings/module/update
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| ratingId | string | 是 | 评分记录ID |
| story | decimal(1) | 否 | 故事性评分（可选，以下同） |
| gameplay | decimal(1) | 否 | 游戏性评分 |
| atmosphere | decimal(1) | 否 | 氛围感评分 |
| value | decimal(1) | 否 | 性价比评分 |
...
### POST /api/v2/ratings/module/delete
...
|--------|------|------|------|
| ratingId | string | 是 | 评分记录ID |
| narration | decimal(1) | 否 | 叙述能力（可选，以下同） |
| improvisation | decimal(1) | 否 | 即兴发挥 |
| fairness | decimal(1) | 否 | 公平性 |
| rhythm | decimal(1) | 否 | 节奏把控 |
| characterPortrayal | decimal(1) | 否 | 角色塑造 |
| atmosphere | decimal(1) | 否 | 氛围营造 |
| content | string | 否 | 文字评价 |
| isAnonymous | bool | 否 | 是否匿名 |
...
### POST /api/v2/ratings/player/submit
...
### POST /api/v2/ratings/player/update
...
|--------|------|------|------|
| ratingId | string | 是 | 评分记录ID |
| roleplay | decimal(1) | 否 | 角色扮演（可选，以下同） |
| cooperation | decimal(1) | 否 | 配合度 |
| ruleFollowing | decimal(1) | 否 | 规则遵守 |
| moodContribution | decimal(1) | 否 | 情绪贡献 |
| punctuality | decimal(1) | 否 | 准时守约 |
| content | string | 否 | 文字评价 |
| isAnonymous | bool | 否 | 是否匿名 |
...
### POST /api/v2/ratings/module/detail/get
...
### POST /api/v2/ratings/gm/detail/get
...
### POST /api/v2/admin/ratings/list
...
### POST /api/v2/admin/ratings/delete
...
### POST /api/v2/votes/create-session
...
### POST /api/v2/admin/votes/detail/get
...
### POST /api/v2/admin/votes/force-close
...
### POST /api/v2/admin/votes/delete
...
### POST /api/v2/follows/me/followers/list
...
### POST /api/v2/follows/status/check
...
### POST /api/v2/admin/follows/stats
...
### POST /api/v2/notifications/unread-count/get
...
### POST /api/v2/venues/rooms/detail/get
...
### POST /api/v2/venues/rooms/calendar/get
...
### POST /api/v2/venues/check-conflict
...
### POST /api/v2/payments/status/get
...
### POST /api/v2/payments/refund-status/get
...
### POST /api/v2/venues/orders/list
...
### POST /api/v2/venues/orders/detail/get
...
### POST /api/v2/venues/orders/adjust-room
...
### POST /api/v2/venues/orders/reschedule
...
|--------|------|------|------|
| orderId | string | 是 | 订单ID |
| newDate | string | 是 | 新日期 |
| newStartTime | string | 是 | 新开始时间 |
| newEndTime | string | 是 | 新结束时间 |
| reason | string | 否 | 操作原因 |
...
### POST /api/v2/venues/orders/confirm-refund
...
|-----------|------|------|------|
| orderId | string | 是 | 订单ID |
| actualRefundAmount | decimal | 是 | 实际退款金额 |
| note | string | 否 | 备注 |
...
### POST /api/v2/venues/revenue/stats
...
### POST /api/v2/admin/venues/rooms/list
...
### POST /api/v2/admin/venues/rooms/create
...
| floor | string | 是 | 楼层 |
| capacity | int | 是 | 容量 |
| hourlyPrice | decimal | 是 | 小时价格 |
| equipment | string[] | 否 | 设备列表 |
| description | string | 否 | 描述 |
| status | string | 否 | 状态：active/maintenance/disabled |
...
### POST /api/v2/admin/venues/rooms/detail/update
...
### POST /api/v2/admin/venues/rooms/status/update
...
|--------|------|------|------|
| roomId | string | 是 | 包间ID |
| status | string | 是 | 状态：maintenance/disabled/active |
| reason | string | 否 | 原因 |
...
### POST /api/v2/auth/permissions/get
...
### POST /api/v2/admin/roles/permissions/update
...
| permissionCodes | string[] | 是 | 权限码数组 |
...
### POST /api/v2/admin/users/roles/update
...
### POST /api/v2/admin/config/rule-systems/list
...
### POST /api/v2/admin/config/rule-systems/create
POST /api/v2/admin/config/rule-systems/detail/update
POST /api/v2/admin/config/rule-systems/delete
...
### POST /api/v2/admin/config/difficulty-levels/list
...
### POST /api/v2/admin/config/orientation-tags/list
...
### POST /api/v2/admin/config/module-types/list
...
| 页面 | 主要依赖接口 |
|------|-------------|
| P01 首页 | POST /sessions/list(recruiting/full) + POST /notifications/unread-count/get + POST /public/config/get |
| P02 模组列表 | POST /modules/list + POST /public/config/get(筛选项) |
| P03 模组详情 | POST /modules/detail/get + POST /ratings/module/detail/get + POST /sessions/list?moduleId= |
| P04 GM主页 | POST /gms/public/get + POST /follows/status/check + POST /ratings/gm/detail/get |
| P05 GM信息编辑 | POST /gms/profile/update |
| P06 开团列表 | POST /sessions/list + POST /sessions/mine/gm/list + POST /sessions/mine/player/list |
| P07 开团详情 | POST /sessions/detail/get + POST /sessions/registrations/list |
| P08 发起开团 | POST /sessions/create + POST /venues/rooms/list + POST /payments/create |
| P09 编辑开团 | POST /sessions/detail/update |
| P10 报名管理 | POST /sessions/registrations/list + POST .../confirm + POST .../cancel |
| P11 改期确认 | POST /sessions/reschedule |
| P12 评分中心 | POST /ratings/my/list |
| P13-P15 评分表单 | POST /ratings/module/submit + POST /ratings/gm/submit + POST /ratings/player/submit |
| P16 评分记录 | POST /ratings/my/list |
| P17-P19 投票 | POST /votes/list + POST /votes/create + POST .../participate + POST .../create-session |
| P20 消息中心 | POST /notifications/list + POST .../read-all |
| P21 包间选择 | POST /venues/rooms/list + POST /check-conflict |
| P22-P23 支付 | POST /payments/create + POST /payments/status/get |
| P24-P28 个人中心 | POST /user/profile/get + POST /follows/gms/list + POST /sessions/mine/player/list |

### Web后台核心页面

| 页面 | 主要依赖接口 |
|------|-------------|
| A01 登录 | POST /auth/web-login |
| A02 仪表盘 | 多个stats接口聚合 |
| A03-A04 用户管理 | POST /admin/users/list + POST .../status/update + POST .../role/update |
| A05 模组列表 | POST /admin/modules/list |
| A06 模组编辑 | POST /modules/create + POST .../detail/update |
| A07 审核队列 | POST /admin/modules/pending-review/list + POST .../review |
| A08 GM管理 | POST /admin/gms/list + POST .../certification/review |
| A09-A10 包间管理 | CRUD /admin/venues/rooms/* + POST .../calendar/get |
| A11-A12 订单管理 | POST /venues/orders/list + POST .../detail/get + POST .../adjust-* |
| A13 前台操作面板 | 同上 + refund接口 |
| A14-A16 配置管理 | CRUD /admin/config/* |
| A17 权限管理 | POST /admin/roles/list + POST .../permissions/update + POST /admin/users/roles/update |
| A18-A20 投票/关注/安全 | 各自admin接口 |

---

*文档生成时间: 2026-07-12  *  总计 **108个POST API接口**，覆盖小程序端28页面 + Web后台20页面的全部数据需求。